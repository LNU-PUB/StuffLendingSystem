...
class TransactionRepositoryTest {
  ...
  void getAllTransactions_ReturnsEmptyList_Initially() {
    assertTrue(((List<Transaction>) repository.getAllTransactions()).isEmpty(),
        "Initially, transactions list should be empty.");
  }
  ...
  void addNewTransaction_AddsTransactionSuccessfully() {
    ...
    assertEquals(1, ((List<Transaction>) repository.getAllTransactions()).size(),
        "Repository should contain one transaction after adding.");
  }
  ...
  void testDeleteTransactionSuccess() {
    ...
    assertTrue(((List<Transaction>) repository.getAllTransactions()).isEmpty(),
        "Transactions list should be empty after deletion.");
  }

  @Test
  void testDeleteTransactionFailure() {
    // Add a transaction to the repository we don't need the return value.
    repository.addNewTransaction(transactionData);
    ...
    assertTrue(((List<Transaction>) repository.getAllTransactions()).size() == 1,
        "Transactions list should contain one transaction after failed deletion.");
  }
  ...
  void updateTransaction_UpdatesTransactionSuccessfully() {
    ...
    Transaction updatedTransaction = new Transaction(added.getId(), updatedData.getMember(), updatedData.getAmount(),
        updatedData.getTransactionDay());
